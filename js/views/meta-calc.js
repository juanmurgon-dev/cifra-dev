...
const PROY = 0.05;        // proyección de crecimiento sobre la semana pasada
...
export function montar(el) {
  ...
  function pintar() {
    ...
    const proj = Math.round(base * (1 + PROY));   // el +5% SIEMPRE se suma aquí
    el.innerHTML = `<div class="card"><h2>Cálculo de meta de compras</h2>${cuerpo(base, proj, pctSel)}</div>`;
...
function cuerpo(base, proj, pct) {
  ...
  const cabeza = `
    ...
    <label class="campo"><span>Venta de la semana pasada</span><input id="vBase" type="number" inputmode="decimal" value="${Math.round(base)}" /></label>`;
  ...
  return cabeza + `
    <div class="aviso-box" style="display:flex;justify-content:space-between;align-items:center;margin-top:8px">
      <span>Proyección <b>+5%</b> sobre la semana pasada</span>
      <b>${money(proj)}</b>
    </div>
    ...
    <div class="sub" style="margin-top:6px">= ${pctInt}% × ${money(proj)}  ·  proyección = ${money(base)} + 5% = ${money(proj)}.</div>
