...
export function montar(el) {
  ...
  el.querySelector("#files").addEventListener("change", async (e) => {
    ...
    // Orden de proceso: primero lo que FIJA LA FECHA del lote, luego lo que la
    // necesita. Los PDF al final (el Excel da la semana exacta para variantes).
    const orden = {
    ...
    let semanaRef = null;
    let diaRef = null;
    let productosCargados = false;   // ¿ya entró un reporte de productos con detalle?   // día detectado en el lote (corte o reporte de 1 día) → acumula variantes por día
      ...
      try {
        ...
        if (it.tipo === "corte") {
          ...
          if (productosCargados) {
            logs.push(`↩️ ${it.f.name}: me lo salté — ya cargué esos productos desde un reporte con más detalle.`);
          } else {
          ...
          // Resumen de extras. Si ya entró un reporte de productos (que trae sus
          // propios modificadores), no lo pises.
          if (productosCargados) {
            logs.push(`↩️ ${it.f.name}: me lo salté — los modificadores ya vinieron en un reporte más completo.`);
          } else {
          ...
          logs.push(`⚠️ ${it.f.name}: no reconocí el formato. Hojas que trae: ${hojas || "—"}. ` +
